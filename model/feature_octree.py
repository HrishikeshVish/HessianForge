...
class FeatureOctree(nn.Module):
    ...
    def query_feature_with_indices(self, coord, hierarchical_indices, return_cat=False):
        ...
        for i in range(self.featured_level_num): # for each level
            ...
            if(return_cat == True):
                if(cat_features is None):
                    cat_features = (self.hier_features[feature_level][hierarchical_indices[i]]*coeffs).unsqueeze(-1)
                else:
                    cur_feature = (self.hier_features[feature_level][hierarchical_indices[i]]*coeffs).unsqueeze(-1)
                    cat_features = torch.cat((cur_feature, cat_features), dim=3)
            else:
    ...
    def query_feature(self, coord, return_cat=False, faster = False):
        ...
        features = self.query_feature_with_indices(coord, indices, return_cat=return_cat)
        return features
