...
class MappingBatchNode(Node):
    def __init__(self):
        super().__init__('mapping_batch_node')
        self.get_logger().info("MappingBatchNode started.")

    def run_mapping_batch(self):
        ...
        if len(sys.argv) > 1:
            ...
            self.get_logger().error(
                ...
                "Try: ros2 run <your_package> hessianforge_batch_ros2 <config.yaml>"
            )
            rclpy.shutdown()
            return

        # Setup experiment and copy config file to run directory
        run_path = setup_experiment(config)
        shutil.copy2(sys.argv[1], run_path)
        dev = config.device

        # Initialize feature octree and physics engine (decoder)
        ...
        # Optionally start visualization if enabled
        if config.o3d_vis_on:
            vis = MapVisualizer()

        self.get_logger().info("Loading, preprocessing, and sampling data")
        for frame_id in tqdm(range(dataset.total_pc_count)):
            if (frame_id < config.begin_frame or 
                frame_id > config.end_frame or 
        ...
        require_gradient = (
            config.normal_loss_on or config.ekional_loss_on or 
        ...
        for iter in progress_bar:
            ...
            if config.consistency_loss_on:
                near_index = torch.randint(
                    0, coord.shape[0],
                    (min(config.consistency_count, coord.shape[0]),), device=dev
                )
            ...
            if config.hessian_loss_on:
                hessian_loss = sdf_hessian_loss(
                    pred, coord, hessian, surface_mask,
...
def main(args=None):
    ...
    node = MappingBatchNode()
    node.run_mapping_batch()
    node.destroy_node()
    rclpy.shutdown()
