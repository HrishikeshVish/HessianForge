...
def setup_optimizer(config: SHINEConfig, octree_feat, phys_eng_param, sigma_size) -> Optimizer:
    lr_cur = config.lr
    opt_setting = []
    # weight_decay is for L2 regularization, only applied to MLP
    if phys_eng_param is not None:
        mlp_geo_param_opt_dict = {'params': phys_eng_param, 'lr': lr_cur, 'weight_decay': config.weight_decay} 
...
def save_decoder(phys_eng, run_path, checkpoint_name):
    torch.save({"phys_eng": phys_eng.state_dict()},
