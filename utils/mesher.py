...
from utils.laplace_optimizer import minimize_laplacian_energy, compute_laplacian_magnitude
...
class Mesher():
        ...
        with torch.no_grad(): # eval step
            if iter_n > 1:
                for n in tqdm(range(iter_n)):
                    ...
                    if(self.phys_eng is not None):

                        batch_feature, crd_feat = self.octree.query_feature(batch_coord, faster=True, return_cat=True)
                    else:
                    ...
                    if query_sdf:
                        if not self.config.time_conditioned:
                            if(self.phys_eng is not None):
                                #if(n == 0):
                                    #print("RECONSTRUCTING FROM NEURAL OPERATOR")
                                batch_sdf = -self.phys_eng(batch_feature)
                                # if(self.config.mc_dropout):
                                #     predictions = []
                                #     for ec in range(5):
                                #         predictions.append(self.phys_eng(feature))
                                #     predictions = torch.stack(predictions, dim=0)
                                #     mean_pred = predictions.mean(dim=0)
                                #     variance_pred = predictions.var(dim=0)
                                #     directions = []
                                #     for ec in range(5):
                                #         directions.append(predictions[ec] - mean_pred)
                                #     directions = torch.stack(directions, dim=0)
                                #     #norm_deviation = torch.norm(directions, dim=0) + 1e-8  # Avoid division by zero
                                #     #pred = pred - torch.sqrt(variance_pred)
                                #     direction = directions.mean(dim=0)
                                #     #direction = F.tanh(direction)
                                #     direction = torch.sign(direction)
                
                                #     #pred =  mean_pred + direction * torch.sqrt(variance_pred)
                                #     mean_pred = mean_pred / (1+ variance_pred)
                                #     sigma_sigmoid = self.config.logistic_gaussian_ratio*self.config.sigma_sigmoid_m*self.config.scale
                                #     occupancy_prob = F.sigmoid(-mean_pred / sigma_sigmoid)
                                #     sdf_pred = -(mean_pred + (2*occupancy_prob-1)*torch.sqrt(variance_pred))
                                #     #sdf_pred = -(mean_pred - direction*torch.sqrt(variance_pred))
                                #     #sdf_pred = -(sdf_pred - torch.sqrt(variance_pred))
                                #     batch_sdf = sdf_pred.detach().cpu().numpy()
                                
                            else:
                                raise NotImplementedError("VCycle Model not defined")
                        else:
                            raise NotImplementedError("Time conditioned sdf not implemented")
                        sdf_pred[head:tail] = batch_sdf.detach().cpu().numpy()
                    if query_sem:
                        raise NotImplementedError("Semantic prediction not implemented")
                    if query_mask:
                ...
                if(self.phys_eng is not None):
                    feature = self.octree.query_feature(coord, faster=True,return_cat=True)
                else:
                    feature = self.octree.query_feature(coord, faster=True)
                if query_sdf:
                    if not self.config.time_conditioned:
                        if(self.phys_eng is not None):
                            #print("RECONSTRUCTING FROM NEURAL OPERATOR")
                            
                            sdf_pred = -self.phys_eng(feature).detach().cpu().numpy()
                            # bump_points, bump_indices= detect_bumps(torch.from_numpy(sdf_pred), coord)
                            # bump_sdf = sdf_pred[bump_indices.detach().cpu().numpy()]
                            # bump_points = bump_points.detach().cpu().numpy()
                            # # Define kernel 
                            # kernel = C(1.0) * RBF(length_scale=1.0)
                            # # Train GPR model
                            # #print(bump_points.shape)
                            # gpr = GaussianProcessRegressor()
                            # gpr.fit(bump_points, bump_sdf)
                            # sdf_pred[bump_indices.detach().cpu().numpy()] = gpr.predict(bump_points)
                            # if(self.config.mc_dropout):

                            #     uncertainty_mask = (variance_pred>0.009)

                            #     uncertain_coords = crd_feat[uncertainty_mask].detach().cpu().numpy()
                            #     if(uncertain_coords.shape[0] >0):
                            #         uncertainty_sd = mean_pred[uncertainty_mask].detach().cpu().numpy()
                            #         gp.fit(uncertain_coords, uncertainty_sd)
                            #         pred_cd = uncertain_coords
                            #         #print(type(pred_cd))
                            #         sdf_pred[uncertainty_mask.detach().cpu().numpy()] = gp.predict(pred_cd)

                                
                                
                            #sdf_pred = -self.phys_eng(feature, crd_feat, eval=True).detach().cpu().numpy()ES
                        else:
                            raise NotImplementedError("VCycle Model not defined")
    ...
    def recon_octree_mesh(self, query_level, mc_res_m, mesh_path, map_path, \
        ...
        # directly use open3d to get mesh
        if(self.config.laplacian_optimization == True):
            laplacian = compute_laplacian_magnitude(torch.from_numpy(verts), torch.from_numpy(faces))
            print(verts.shape)
            ...
            V = minimize_laplacian_energy(verts, faces, laplacian)
            new_laplacian = compute_laplacian_magnitude(V, torch.from_numpy(faces))
